...
(defcustom emms-cover-savepath "~/.covers/%artist - %album.%size.%ext"
  "The path to store and find the cover files. %path %artist, %album, %size and %ext will be replaced."
  :group 'emms-cover
  :type 'string)

(defcustom emms-cover-savepath-original "%path/folder.%ext"
  "The path to store and find the unmodified cover files. %path %artist, %album and %ext will be replaced."
  :group 'emms-cover
  :type 'string)
	...
	(let
	    ((artist (match-string (nth 1 emms-cover-pathregexp) clean-path))
	     (album (match-string (nth 2 emms-cover-pathregexp) clean-path)))
          ...
          (or (emms-cover-find-existing path artist album size)
              (unless (emms-cover-in-queue artist album)
                (message "Downloading Album Cover for %s album %s" artist album)
                (emms-cover-download path artist album size)))))))

(defun emms-cover-create-path (path artist album size &optional extension)
  (let ((cover (if (eq size 'large)
                   emms-cover-savepath-original
                 emms-cover-savepath)))

    (setq cover (replace-regexp-in-string "%artist" artist cover))
    (setq cover (replace-regexp-in-string "%album" album cover))
    (setq cover (replace-regexp-in-string "%path" path cover))
    (setq cover (replace-regexp-in-string "%size"
                                          (symbol-name size) cover))
    (if extension
        (setq cover (replace-regexp-in-string "%ext" extension
                                              cover)))
    cover))

(defun emms-cover-find-existing (path artist album size)
  ...
  (catch 'emms-cover-find
    (let ((cover (emms-cover-create-path path artist album size))
          attempt-path)
...
(defun emms-cover-download (path artist album size)
  ...
  (catch 'emms-cover-download
    (dolist (func emms-cover-download-functions)
      (let ((url (funcall func artist album))
            (coverpath))
	  (when url
              (setq coverpath (emms-cover-download-image url path artist album))
              (emms-cover-remove-from-queue artist album)
	      (throw 'emms-cover-download
		     (emms-cover-find-existing path artist album
                                               size)))))
    nil))
...
(defun emms-cover-download-image (url path artist album)
  (let ((extension (downcase (file-name-extension
                              url)))
        coverpath
        largecover)

    (setq largecover (emms-cover-create-path path artist album
                                             'large extension))
    ...
    (dolist (size '(medium small))
      (setq coverpath (emms-cover-create-path path artist album
                                              size extension))
      (unless (file-exists-p coverpath)
	(let ((dimensions (cdr (assoc size emms-cover-dimensions))))
	  (emms-cover-resize-image largecover coverpath
                                   (nth 0 dimensions)
                                   (nth 1 dimensions)))))
    coverpath))
