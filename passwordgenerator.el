...
(defun make-password-char-list (upper lower number symbol ambiguous)
  ...
  (when ambiguous
      (if upper
	  (setq char-list (append char-list upper-chars-ambiguous)))
      (if lower
	  (setq char-list (append char-list lower-chars-ambiguous)))
      (if number
	  (setq char-list (append char-list number-chars-ambiguous)))
      (if symbol
	  (setq char-list (append char-list symbol-chars-ambiguous)))
      )
  char-list))
...
(defun generate-password () 
  "Generate a new password and insert it at point"
  (interactive)
  (insert  (apply 'make-password (make-password-prompt-for-args))))
